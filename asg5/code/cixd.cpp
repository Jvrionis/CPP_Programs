...
void reply_put(accepted_socket& client_sock, cix_header& header){
    try {
        ofstream file( header.filename, std::ios::out | 
                                        std::ios::binary );
        char *data = new char[header.nbytes] ();

        client_sock.recv(data, header.nbytes);
        file << data;
        header.command = cix_command::ACK;
        header.nbytes = 0;
        send_packet (client_sock, &header, sizeof header);
...
void reply_get(accepted_socket& client_sock, cix_header& header){
    ifstream file( header.filename, std::ios::binary | std::ios::ate );
    ...
    if (size != -1)
    {
        ...
        if(file.read(data, size))
        {
            ...
            header.nbytes = size;
            send_packet (client_sock, &header, sizeof header);
            send_packet (client_sock, data, size);
            file.close();
            return;
        }
    }
